...
function encodePacketIntoTransmitFormat(filePath, sessionAgreedSyncKey, subSyncKey, clientNetworkId, clientPublicKey, personalPrivateKey, cb) {
    ...
    var buffers = [];
    // var output = fs.createWriteStream(`${filePath}.enc`);

    // Pipe hybridSyncToken into output
    buffers.push(hybridSyncToken)
    // const readableHybridSyncToken = new Readable();
    // readableHybridSyncToken._read = () => {};
    // readableHybridSyncToken.push(hybridSyncToken);
    // readableHybridSyncToken.pipe(output);

    // Pipe timestamp into cipher
    buffers.push(Buffer.from(creationTimestamp))
    // const readableTimestamp = new Readable();
    // readableTimestamp._read = () => {};
    // readableTimestamp.push(creationTimestamp);
    // readableTimestamp.pipe(output);

    // Pipe random bytes into cipher
    buffers.push(encRandomBytes)
    // const readableEncRandomBytes = new Readable();
    // readableEncRandomBytes._read = () => {};
    // readableEncRandomBytes.push(encRandomBytes);
    // readableEncRandomBytes.pipe(output);

    // Pipe iv bytes into cipher
    buffers.push(iv)
    // const readableIVBytes = new Readable();
    // readableIVBytes._read = () => {};
    // readableIVBytes.push(iv);
    // readableIVBytes.pipe(output);

    // Pipe file contents into cipher
